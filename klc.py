...
size_t KLCNString_msize(KLCNString *s) {
  return s->size;
}
...
KLCNString* KLCNstr(KLC_var v) {
  switch (v.tag) {
    case KLC_TAG_INT: {
      char buffer[50];
      sprintf(buffer, "%d", v.u.i);
      return KLC_mkstr(buffer);
    }
    case KLC_TAG_LONG: {
      char buffer[50];
      sprintf(buffer, "%ld", v.u.l);
      return KLC_mkstr(buffer);
    }
    case KLC_TAG_SIZE_T: {
      /* TODO: casing size_t to unsigned long can have issues */
      /* Address this */
      char buffer[50];
      sprintf(buffer, "%lu", (unsigned long) v.u.s);
      return KLC_mkstr(buffer);
    }
    case KLC_TAG_DOUBLE: {
      char buffer[50];
...
node = Program(tok, [
    ...
    FunctionDefinition(tok, 'size_t', 'String_msize',
